...
output_path = "cement_demand_with_population.xlsx"
...
def read_gdp_projection(path):
    ...
    required_columns = {"Country", "ISO2", "ISO3"}
...
def latest_positive_base_year(row, candidate_years):
    """
    Return the latest year with a positive historical cement value.
    """
    for year in reversed(candidate_years):
        if year in row.index and pd.notna(row[year]) and row[year] > 0:
            return year
...
def project_cement_demand(cement_rows, population_rows, gdp, start_year, end_year):
    """
    Project annual cement demand using GDP-per-capita growth and income elasticity.

    The state variable is cement output per capita in tonnes per person. Because
    cement demand is in kt and population is in thousands of persons, kt/thousand
    persons is numerically equivalent to tonnes/person.
    """
    projected = cement_rows.copy()

    gdp_by_iso3 = gdp.set_index("ISO3")
    population_by_iso3 = population_rows.set_index("ISO3")
    gdp_years = [col for col in gdp_by_iso3.columns if isinstance(col, int)]
    if not gdp_years:
        raise ValueError("GDP projection file does not contain integer year columns.")

    first_gdp_year = min(gdp_years)
    historical_years = list(
        range(max(START_YEAR, first_gdp_year), min(BASE_YEAR, start_year - 1) + 1)
    )
    ...
    for idx, row in projected.iterrows():
        ...
        base_year = latest_positive_base_year(row, historical_years)
...
# This is the authoritative source for country name, ISO2 and ISO3.
# -------------------------------------------------------------------
country_ref = (
...
# Keep only ISO3-like country columns
cement_long = cement_long.dropna(subset=["ISO3"]).copy()

# Remove empty demand values
...
# -------------------------------------------------------------------
# Project cement demand from GDP per capita, population and elasticity
# -------------------------------------------------------------------
cement_rows, projection_issues, projection_imputations = project_cement_demand(
    cement_rows=cement_rows,
    population_rows=population_rows,
    gdp=gdp,
...
print(f"Cement projection years filled: {PROJECTION_START_YEAR}-{END_YEAR}")
